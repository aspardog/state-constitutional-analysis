...
This project is designed to run with **[Claude Code](https://docs.claude.com/en/docs/claude-code)** for the LLM-based evaluation step.
...
```
┌─────────────────┐      ┌──────────────────────┐      ┌──────────────────┐
│  01_scrape.py   │ ───> │  Claude Code evaluates │ ───> │ 02_build_output  │
│  (one-time)     │      │  (one-time, manual)    │      │  (run anytime)   │
└─────────────────┘      └──────────────────────┘      └──────────────────┘
        │                          │                            │
        v                          v                            v
...
- **Scraping is deterministic and cached.** Run once; re-run only if you need to refresh.
...
├── code/
│   ├── 01_scrape.py                Scraper entrypoint
...
│   └── scores.xlsx                 GENERATED by Claude Code (canonical)
├── data/
│   └── raw/                        Cached constitution text (gitignored)
└── output/
    └── State_Constitutional_Analysis.xlsx   GENERATED final deliverable
```
...
### Step 1 — Scrape the constitutions (one-time)

```bash
python code/01_scrape.py
```

This downloads each state's constitution and saves it to `data/raw/{state_name}.txt` (e.g., `south_carolina.txt`, `tennessee.txt`).

If scraping fails for a state (sites change, networks block), the script prints a clear message. You can manually copy-paste the constitution text into `data/raw/{state_code}.txt` and the rest of the pipeline will work.
...
Tests cover:
- The Excel builder produces a valid workbook with correct sheet names and zero formula errors
- The scraper config loads correctly
